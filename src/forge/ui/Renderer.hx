...
class Renderer {
	...
	public function new(state:AppState, provider:IDataProvider, onChampClick:(String) -> Void, onModeClick:(String) -> Void,
			onFilterClick:(FilterType) -> Void, onChampSearch:(String) -> Void, onItemSearch:(String) -> Void) {
		this.state = state;
	...
	public function renderChampionInfoBar():Void {
		...
		if (profile != null) {
			...
			if (profile.hasApScaling)
				badges.push('<span class="badge badge-ap">AP scaling</span>');
			if (profile.hasAdScaling)
				badges.push('<span class="badge badge-ad">AD scaling</span>');
			if (profile.hasOnHit)
				badges.push('<span class="badge badge-onhit">On-hit</span>');
			if (profile.hasDash)
				badges.push('<span class="badge badge-mobility">Dash</span>');
			if (profile.hasShield)
				badges.push('<span class="badge badge-shield">Shield</span>');
			if (profile.hasHeal)
				badges.push('<span class="badge badge-heal">Heal</span>');
			if (profile.hasCc)
				badges.push('<span class="badge badge-cc">CC</span>');
			if (profile.hasExecute)
				badges.push('<span class="badge badge-execute">Execute</span>');
			if (profile.hasStealth)
				badges.push('<span class="badge badge-stealth">Stealth</span>');
			if (profile.isManaless)
				badges.push('<span class="badge badge-manaless">Manaless</span>');
	...
	public function renderItemsToolbar():Void {
		...
		cast(itemsToolbar, Element).style.display = visible ? "flex" : "none";
		if (!visible)
			return;
	...
	public function renderItemsGrid():Void {
      ...
      <div class="synergy-stats">
		...
		for (cat in ItemCategoryTools.displayOrder) {
			...
			var catClass = catLabel.toLowerCase();
			html.add('<div class="category-section">');
			html.add('<div class="category-header cat-$catClass"><span class="cat-label">$catLabel</span> <span class="cat-count">${catItems.length}</span></div>');
			html.add('<div class="items-grid">');
          ...
          <div class="item-card $tierClass"
			...
			html.add('</div>'); // items-grid
			html.add('</div>'); // category-section
		}
	...
	function showTooltip(e:Event, el:Element):Void {
		...
		if (reasonList.length > 0) {
			var items = reasonList.map(function(r) {
				var cls = r.charAt(0) == "+" ? "pos" : "neg";
				return '<li class="$cls">$r</li>';
			}).join("");
			var tierCls = tier.indexOf("core") >= 0
				|| tier.indexOf("good") >= 0 ? "pos" : tier.indexOf("weak") >= 0 || tier.indexOf("trap") >= 0 ? "neg" : "neu";
			...
			if (label != "") {
				reasonHtml = '<div class="tt-reasons $tierCls"><strong>$label</strong><ul>$items</ul></div>';
			}
		...
		// Champion kit summary relevant to synergy
		var champKitHtml = "";
		if (state.champProfile != null) {
			var p = state.champProfile;
			...
			if (p.hasHeal)
				traits.push("Heal");
			if (p.isManaless)
				traits.push("Manaless");
			if (traits.length > 0) {
				champKitHtml = '<div class="tt-kit">${p.name}: ${traits.join(" · ")}</div>';
			}
		}
	...
	function positionTooltip(e:MouseEvent):Void {
		var tt:Element = cast tooltip;
		var pad = 14;
		var x = e.clientX + pad;
		var y = e.clientY + pad;
		if (x + 280 > js.Browser.window.innerWidth)
			x = e.clientX - 280 - pad;
		if (y + tt.offsetHeight > js.Browser.window.innerHeight)
			y = e.clientY - tt.offsetHeight - pad;
		tt.style.left = '${x}px';
		tt.style.top = '${y}px';
	}
	...
	function bindStaticEvents():Void {
		...
		// Refresh data button
		var refetchBtn = js.Browser.document.getElementById("refetchChampions");
		if (refetchBtn != null) {
			refetchBtn.addEventListener("click", function(_) {
				if (forge.Main != null)
					forge.Main.refreshData();
			});
		}
		...
		// Champion search
		var champSearchEl:InputElement = cast js.Browser.document.getElementById("champSearch");
		...
		// Item search
		var itemSearchEl:InputElement = cast js.Browser.document.getElementById("itemSearch");
